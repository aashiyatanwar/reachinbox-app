import { BarChart2, Home, Inbox as InboxIcon, List, Mail, Send, UserRoundSearch as UserRoundSearchIcon } from 'lucide-react';
...
const Sidebar = ({ currColor, username, handleChange }) => {
  ...
  return (
    <div className={`w-full h-full flex flex-col justify-between ${currColor ? "bg-[#101113]" : "bg-white"} px-1`}>
      ...
      <div className='w-12  h-[70px] flex justify-center items-center'>
        <p className={`w-8 p-1 m-7 bg-green-900 h-8 rounded-3xl text-white `}>{username}</p>
      </div>
    </div>
  );
};
